...
def train_one_epoch(model, loader, criterion, optimizer, device, scaler=None):
    ...
    return avg_loss, acc
@torch.no_grad()
def predict_files(model, files, classes, dataset, img_size, device, topk=5):
    try:
        import torchvision
...
def main():
    ...
    if args.test_only and not args.resume:
        default_best = os.path.join(args.save_dir, "best_model_simple_cnn.pt")
        if os.path.exists(default_best):
            args.resume = default_best
        else:
            legacy_best = os.path.join(args.save_dir, "best_model.pt")
            if os.path.exists(legacy_best):
                args.resume = legacy_best
    if args.resume:
    ...
    if args.predict or args.predict_dir:
        ...
        if not files:
            ...
            return
        predict_files(model, files, classes, args.dataset, args.img_size, device, topk=args.topk)
        return
    if args.test_only:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        ckpt_path = os.path.join(args.save_dir, "checkpoint_simple_cnn.pt")
        torch.save({
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            best_path = os.path.join(args.save_dir, "best_model_simple_cnn.pt")
            torch.save({"model_state": model.state_dict(), "num_classes": num_classes}, best_path)
            print(f"saved={best_path} best_acc={best_acc:.4f}")
    final_path = os.path.join(args.save_dir, "last_model_simple_cnn.pt")
